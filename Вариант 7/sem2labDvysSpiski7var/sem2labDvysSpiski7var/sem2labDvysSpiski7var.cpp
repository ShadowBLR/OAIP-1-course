...
int menu(void)
{
	...
	cout << "7. Поиск макс значения" << endl;
	cout << "8. Выход" << endl;
	cout << endl;
	do
	{
...
//-----------------------------------------------------------
void search(char name[NAME_SIZE], Address** phead)    // Поиск имени в списке
{
	Address* t = *phead;
	while (t)
	{
...
void searchdMax(Address **pHead)
{
	...
	case 1:
		while (t)
		{
			if (strlen(t->name) > strlen(temp->name))
				temp = t;
			t = t->next;
		}
		break;
	case 2:
		while (t)
		{
			if (strlen(t->city) > strlen(temp->city))
				temp = t;
			t = t->next;
		}
		break;
