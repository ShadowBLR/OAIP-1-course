...
void leaves(Tree* t, int& size);
int c = 0;         //количество слов
Tree* Root = NULL; 	//указатель корня
void sum(Tree* t,int &sm);
void main()
{
	setlocale(0, "Russian");
	int key, choice, n, size = 0,sm=0;
	...
	for (;;)
	{
		...
		case 8:leaves(Root, size); cout << "Число листьев = " << size << endl; break;
		case 9:sum(Root, sm); cout << "\nСумма = " << sm << endl; break;
		case 10:  exit(0);
		}
	}
}
...
void leaves(Tree* t, int& size)
{
	...
	else
	{
		if (t->Right == NULL && t->Left == NULL)
			size++;
		leaves(t->Left, size);     //левое поддерево
	}
	leaves(t->Right, size);          // правое поддерево
}
Tree* insertElem(Tree* t, int key, char* s)  //Добавление нового элемента
{
	...
	if (!find&&Prev!=NULL)              //найдено место с адресом Prev
	{
...
void sum(Tree* t, int& sm)
{
	...
	{	if(t->Left!=NULL||t->Right!=NULL)
		sm += t->key;
		sum(t->Right,sm);	//вывод правого поддерева
		sum(t->Left,sm) ;	//вывод левого поддерева
	}
}
